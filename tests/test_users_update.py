...
def test_user_can_update_own_profile(client, db_session):
    user, token = create_user_and_token(client, db_session, "u1@example.com", "secret123")
    response = client.put(
        f"/users/{user.id}",
...
def test_user_cannot_update_other_profile(client, db_session):
    u1, token1 = create_user_and_token(client, db_session, "u1@example.com", "secret123")
...
def test_admin_can_update_any_profile(client, db_session):
    admin, token_admin = create_user_and_token(client, db_session, "admin@example.com", "adminpass", is_admin=True)
